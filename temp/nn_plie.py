...
param = {'bst:max_depth':4,'bst:eta':1,'silent':1,'objective':'reg:linear'}
param['nthread'] = 4
evallist = [(dtrain,'train')]
plst = param.items()
bst = xgb.train(plst,dtrain,1000,evallist)
...
for i in range(15):
    clf.fit(train,labels)
    valid_pred = clf.predict(valid)
    print "valid:"
    score_last = score
    score = metrics.quadratic_weighted_kappa(valid_labels,valid_pred)
    print score
...
submission.to_csv('nn_2hidden.csv', index=False)
